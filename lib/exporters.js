...
function getPhantomjsExporter(ext) {
  ...
  return function phantomjsExporterFn (options, cb) {
    ...
    // TODO: Don't make this sync
    tmp.writeFileSync(encodedArg, 'utf8');

    // Create a child process for phantomjs
    var phantomjs = spawn(phantomLocation, [path.join(__dirname, 'scripts', 'compose.js'), filepath]);
    ...
    // When we are done
    phantomjs.on('close', function handleClose (code) {
      ...
      if (code !== 0) {
        var err = new Error('Received non-zero exit code "' + code + '" from PhantomJS. stderr:' + stderr);
        return cb(err);
      }
      ...
      if (ext === 'jpeg') {
        ...
        // Convert its internal buffer into a binary string (expected by spritesmith)
