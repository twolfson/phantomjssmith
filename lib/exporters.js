...
function getPhantomjsExporter(ext) {
  ...
  return function phantomjsExporterFn (options, cb) {
    ...
    // TODO: Rename this to `stdout` and alias it to `encodedPixels` later on
    ...
    phantomjs.on('close', function handleClose () {
      ...
      // TODO: Delete this and use `save-pixels` instead
      if (ext === 'jpeg') {
        var jpg;
        try {
          jpg = jpegJs.encode({
