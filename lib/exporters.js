...
function getPhantomjsExporter(ext) {
  ...
  return function phantomjsExporterFn (options, cb) {
    ...
    phantomjs.on('close', function () {
      ...
      var i = 0,
          len = encodedPixels.length,
          decodedPixels = new Uint8Array(len);
      for (; i < len; i += 4) {
        decodedPixels[i] = encodedPixels.charCodeAt(i) - 33;
        decodedPixels[i] = encodedPixels.charCodeAt(i + 1) - 33;
        decodedPixels[i] = encodedPixels.charCodeAt(i + 2) - 33;
        decodedPixels[i] = encodedPixels.charCodeAt(i + 3) - 33;
      }
      ...
      // console.log(png);
      png.pipe(require('fs').createWriteStream('test.png'));
    });
  };
}
