...
function createImages(files, cb) {
  ...
  async.waterfall([
    ...
    function getImgSize (cb) {
      ...
      console.log('phantomjs', [path.join(__dirname, 'scripts', 'stats.js'), filepath]);
      bufferedSpawn('phantomjs', ['--web-security=no', path.join(__dirname, 'scripts', 'stats.js'), filepath],
    ...
    function saveImgSize (cb) {
      ...
      var dimensionArr;
      try {
        dimensionArr = JSON.parse(stdout);
      } catch (e) {
        return cb(new Error('Error while parsing JSON "' + stdout + '".\n' + e.message));
      }
